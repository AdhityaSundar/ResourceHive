...
export function FaqPageClient() {
  ...
  return (
    <div className="mx-auto max-w-5xl px-4 py-14 sm:px-6 lg:px-8">
      <h1 className="text-5xl font-black text-[#2a1833]">{messages.faq.title}</h1>
      <div className="mt-8 space-y-4">
        {faqs.map((item) => (
          <details key={item.question} className="glass-panel rounded-[28px] p-6">
            <summary className="cursor-pointer list-none text-lg font-bold text-[#2a1833]">
              {item.question}
            </summary>
            <p className="mt-4 text-sm leading-7 text-[#7c6b88]">{item.answer}</p>
          </details>
        ))}
      </div>
    </div>
